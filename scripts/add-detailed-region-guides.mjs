...
const topicParticle=value=>{const code=value.charCodeAt(value.length-1);return code>=0xac00&&code<=0xd7a3&&(code-0xac00)%28!==0?'은':'는';};
...
let changed=0;
for(const city of dirs){const file=path.join(root,city,'index.html');let html=fs.readFileSync(file,'utf8');const block=render(city,html);if(html.includes(marker)){const next=html.replace(/<section class="detailed-region-guide" data-detailed-region-guide="v1">[\s\S]*?<\/section>(?=<section class="card)/,block);if(next!==html){fs.writeFileSync(file,next);changed++;}continue;}const anchors=['<section class="card case"','<section class="card" id="area"','<section class="card" id="cases"'];const anchor=anchors.find(item=>html.includes(item));if(!anchor)continue;html=html.replace(anchor,`${block}${anchor}`);fs.writeFileSync(file,html);changed++;}
